...
namespace SEPImageClassification
{
    public class SEPExperiment
    {
        public List<List<string>> MainTestResultList = new List<List<string>>();

        readonly HtmConfig htmConfig;
        readonly SEPArgsConfig.SEPArgsConfig expConfig;

        /// <summary>
        /// Setiing the initialized parameters of the HTM 
        /// </summary>
        public SEPExperiment(SEPArgsConfig.SEPArgsConfig config)
        {
            expConfig = config;
        ...
        public Dictionary<string, string> Run()
        {
            ...
            SEPHelpersTemp helperFunc = new SEPHelpersTemp();
            ...
            string testdir = Path.Combine(Directory.GetCurrentDirectory() , "TestImg");
            ...
            for (int i = 0; i < testfiles.Length; i++)
            {
                ...
                ////mirroring the output into text file
                FileStream ostrm;
                StreamWriter writer;
                TextWriter oldOut = Console.Out;
                var fileName = "Redirect2.txt";
                try
                {
                ...
                (string predictedLabel, double maxPredictedSimilarity) = PredictLabel(sdrOfInputImage, sdrs, inputsPath, testdir);
                result.Add("predictedLabel", predictedLabel);
                result.Add("testfilesname", testfilesname);
                result.Add("maxPredictedSimilarity", maxPredictedSimilarity.ToString());

                Console.WriteLine($"Input Test Image {testfilesname} is predicted as {predictedLabel}\n");

                //cutting the mirroring process
                ...
                using (Stream input = File.OpenRead("Redirect2.txt"))
                using (Stream output = new FileStream("Redirect.txt", FileMode.Append,
                                                      FileAccess.Write, FileShare.None))
                {
                    input.CopyTo(output);

                    Console.ForegroundColor = ConsoleColor.Blue;
                    Console.WriteLine("Merging done successfully");
                    Console.ResetColor();
                }
                File.Delete("Redirect2.txt");


                //////////////////here should we read from the console   
            }
            return result;
        }
        #region
        ...
        (string, double) PredictLabel(int[] sdrInputTestImage, Dictionary<string, int[]> sdrs, Dictionary<string, List<string>> inputsPath, string testdir)
        {
            ...
            foreach (KeyValuePair<string, List<string>> secondEntry in inputsPath)
            {
                ...
                TestResultList.Add(secondEntry.Key);
                TestResultList.Add(maxSimilarity.ToString());
                TestResultList.Add(avgSimilarity.ToString());
                TestResultList.Add(minSimilarity.ToString());

                List<string> copy = TestResultList.ToList();


                MainTestResultList.Add(copy);
                TestResultList.Clear();
                Console.WriteLine("lengt" + MainTestResultList.Count);
                foreach (var item in MainTestResultList)
                {
            ...
            return (predictedLableCat, maxPredictedSimilarity);
